...
def read_data(vocab, maxlen, path=None, domain=None, phase=None):
    ...
    print 'Unknown words in %s: %.2f ' % (phase, unk_hit) 
    logging.info('Unknown words in %s: %.2f '%(phase, unk_hit))
    print 'Total words in %s: %.2f ' % (phase, total) 
    logging.info('Total words in %s: %.2f '%(phase, total))

    # print 'Unknown index and top rows of data: %s' % (vocab['<unk>'])
    # print data[:10]
...
def prepare_data(domain, vocab_size, use_doc, maxlen=0):
    assert domain in ['res', 'lt', 'res_15', 'norec_fine']  # NOTE added norec_fine
