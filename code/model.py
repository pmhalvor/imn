...
def create_model(args, vocab, nb_class, overall_maxlen, doc_maxlen_1, doc_maxlen_2):
    ...
    def init_emb(emb_matrix, vocab, emb_file_gen, emb_file_domain):
        ...
        # NOTE added gen_dim logic for handling norec_fine emb_dim = 100
        gen_dim = args.emb_dim - 100 if args.use_domain_emb else args.emb_dim

        for line in pretrained_emb:
            tokens = line.split()
            if len(tokens) != gen_dim+1:
                continue
            word = tokens[0]
            vec = tokens[1:]
            try:
                emb_matrix[0][vocab[word]][:gen_dim] = vec
                counter_gen += 1
            except KeyError:
                pass
